(async function() {
  ...
  // === Extraction LinkedIn ===
  // Parse le titre depuis document.title : "(N) Titre | LinkedIn" → "Titre"
  ...
  function extractLinkedIn() {
    ...
    // 2) Si aucun sélecteur Pulse ne matche (DOM logué auteur), fallback Readability
    if (!articleEl) {
      const titleEl = document.querySelector('meta[property="og:title"]');
      const ogImage = document.querySelector('meta[property="og:image"]');
      const articleTitle = (titleEl && titleEl.content) ? titleEl.content : parseLinkedInDocTitle(document.title);
      ...
      // Dernier recours : article élément générique
  ...
  try {
    ...
    // (LinkedIn affiche un DOM différent quand on est l'auteur logué)
    if (source === 'generic' || source === 'linkedin') {
